 ...
 * 
 * Copyright 2010, Gilbert Pellegrom Free to use and abuse under the MIT
	...
	var a = function(j, q) {
		...
		var p = function(r, s, u, x) {
			...
			if (u.controlNav) {
				...
				b(".nivo-controlNav a:eq(" + y.currentSlide + ")", r)
						.addClass("active")
			}
			...
			b(".nivo-slice", r).each(function() {
				...
				b(this).css({
					height : "0px",
					opacity : "0",
					background : "url(" + y.currentImage.attr("src")
							+ ") no-repeat -" + ((i + (w * i)) - i) + "px 0%"
				});
				w++
			});
			if (u.effect == "random") {
