/**
 * PowerviaWindow - (1.2 $Date: 2008-06-24 14:22:56 +0800 (星期二, 2008-06-24) $ base jQuery), also need jquery.myext.js
 *
 * please reserving all of the comment, otherwise you should not use those code
 *
 * @Author    muqiao
 * @Version   1.2
 * @Copyright Edit from original prototype-window(http://xilinus.com) By muqiao(http://powervia.com)
 * @Revision  $Id: pvWindow_src.js 39 2008-06-24 06:22:56Z emkiao@qq.com $
 */
(function($){
var namespace = arguments[1]||'Dialog';
	...
	info:function(message,timeout,options){
		...
		if(timeout){
			options.onShow = function(){
				setTimeout(this.close.bind(this),$.toInt(timeout,5000));
			};
		}
		return Dialogs.opendialog(message,options);
	},
...
var _createWin = function(obj){
	...
	if(o.url){
		...
		obj.content = $('<div class="dialog_content" style="background-color: '+bgcolor+';">'+(o.content||'&nbsp;')+'</div>');
	}
	...
	o.closable &&
		$('#'+obj.id+'_close',obj.dialog).bind("mouseover",mouseover).bind("mouseout",mouseout).bind("mousedown",mousedown).bind("mouseup",mouseover).bind("click",obj.close.bind(obj));
	o.minimizable &&
		$('#'+obj.id+'_minimize',obj.dialog).bind("mouseover",mouseover).bind("mouseout",mouseout).bind("mousedown",mousedown).bind("mouseup",mouseover).bind("click",obj.minimize.bind(obj));
	o.maximizable &&
		$('#'+obj.id+'_maximize',obj.dialog).bind("mouseover",mouseover).bind("mouseout",mouseout).bind("mousedown",mousedown).bind("mouseup",mouseover).bind("click",obj.maximize.bind(obj));
	var buttons = '';
	if(o.button){
		Dialogs.calls[obj.id] = [];
		buttons = '<div id="'+obj.id+'_button" class="dialog_buttons" style="background-color: '+bgcolor+';">';
		for(var key in o.button){
			...
			Dialogs.calls[obj.id][l] = o.button[key][1].bind(obj);
		}
		buttons += '</div>';
	}
	$('#' + obj.id + "_table_content",obj.dialog).append(obj.content).append(buttons);
	typeof o.onload == 'function' &&
		obj.content.bind('load',o.onload.bind(obj));
	if(o.draggable || o.resizable){
		obj.initDrag = _initDrag.bindE(obj);
		obj.endDrag  = _endDrag.bindE(obj);
		obj.updateDrag = _updateDrag.bindE(obj);
	...
	o.maximizable && $('#'+obj.id+'_top',obj.dialog).parent().bind("dblclick",obj.maximize.bind(obj));
	o.resizable && (obj.sizer = $('#'+obj.id+'_sizer',obj.dialog).bind("mousedown",obj.initDrag));
	obj.content.bind("mousedown",obj.toFront.bind(obj));
